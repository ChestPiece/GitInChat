  ...
  return (
    <div ref={rootRef} className="flex flex-col items-center justify-center h-full gap-8">
      <div className="text-center">
        <div className="w-16 h-16 glass rounded-lg flex items-center justify-center mx-auto mb-4 border border-[var(--gh-border)]">
          <svg className="w-9 h-9 text-[var(--gh-text-muted)]" viewBox="0 0 98 96" fill="none" aria-hidden="true">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-3xl px-4">
          {SUGGESTED_PROMPTS.map(({ icon: PromptIcon, label, prompt }) => (
            <button
              ...
              className="suggest-chip flex items-center gap-2 px-4 py-3 rounded-lg border border-[var(--gh-border)] bg-white/[0.02] hover:border-[rgba(35,134,54,0.3)] hover:shadow-[0_0_20px_var(--gh-green-glow)] text-sm text-left text-muted-foreground hover:text-foreground transition-all"
            >
