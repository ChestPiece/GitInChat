...
export function validateRepoInput(owner: string | undefined, repo: string | undefined): { valid: boolean; error?: string } {
  if (!owner || !repo) {
    return { valid: false, error: 'Owner and repo are required' };
  }
  if (!VALIDATE_REPO_REGEX.test(owner)) {
    return { valid: false, error: 'Invalid owner format - only alphanumeric, hyphens, underscores, dots allowed' };
  }
  if (!VALIDATE_REPO_REGEX.test(repo)) {
    return { valid: false, error: 'Invalid repo format - only alphanumeric, hyphens, underscores, dots allowed' };
  }
  return { valid: true };
}

export function validatePathInput(path: string | undefined): { valid: boolean; error?: string } {
  if (!path) {
    return { valid: true }; // empty path is ok (root)
  }
  if (!VALIDATE_PATH_REGEX.test(path)) {
    return { valid: false, error: 'Invalid path format - path traversal attempts detected' };
  }
  if (path.includes('..') || path.includes('\\')) {
    return { valid: false, error: 'Path traversal not allowed' };
  }
  if (path.startsWith('.git') || path.includes('/.git') || path.includes('\\.git')) {
 ...
 */
export function sanitizeStringParam(value: string | undefined, paramName: string): { valid: boolean; error?: string; value?: string } {
  if (!value) {
  ...
  if (trimmed.length > 1000) {
    return { valid: false, error: `${paramName} exceeds maximum length of 1000 characters` };
  }
