...
async function runVerification() {
  ...
  try {
    ...
    const createResult = await createRepositoryTool.execute?.(
      ...
      { toolCallId: "verify-create", messages: [] },
    );
    ...
    const actualOwner =
      createResult.data?.owner ||
      createResult.data?.full_name?.split("/")[0] ||
      owner;
    const actualName = createResult.data?.name; // Should be sanitized

    if (createResult.data?.warning) {
      console.log(
        "✅ SUCCESS: Creation warning present:",
        createResult.data.warning,
      );
    } else {
      console.log(
        "⚠️ WARNING: No sanitization warning on creation (maybe name didn't need it?)",
      );
    }
    ...
    const updateResult = await updateRepositoryTool.execute?.(
      ...
      { toolCallId: "verify-update", messages: [] },
    );
    ...
    if (updateResult.data?.warning) {
      console.log(
        "✅ SUCCESS: Update warning present:",
        updateResult.data.warning,
      );
    } else {
    ...
    const deleteFailResult = await deleteRepositoryTool.execute?.(
      ...
      {
        toolCallId: "verify-delete-fail",
        messages: [{ role: "user", content: "wrong-name" }],
      },
    );

    console.log(
      "Delete (Expected Fail) Result:",
      JSON.stringify(deleteFailResult, null, 2),
    );
    ...
    const deleteSuccessResult = await deleteRepositoryTool.execute?.(
      ...
      {
        toolCallId: "verify-delete-success",
        messages: [{ role: "user", content: updatedName }],
      },
    );

    console.log(
      "Delete (Success) Result:",
      JSON.stringify(deleteSuccessResult, null, 2),
    );
