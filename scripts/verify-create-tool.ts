...
async function run() {
  ...
  await successTool.execute?.(
    { name: "World" },
    { toolCallId: "test-success", messages: [] },
  );
  ...
  try {
    const result = await errorTool.execute?.(
      { shouldFail: true },
      { toolCallId: "test-error", messages: [] },
    );
    console.log("Error tool result (should be safe error object):", result);
  } catch (e) {
    console.error(
      "CRITICAL: Tool threw exception instead of returning error object!",
      e,
    );
  }
}
